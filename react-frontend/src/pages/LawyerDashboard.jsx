...
import { Briefcase, Search, RefreshCw, LogOut } from 'lucide-react';
import CaseDetails from '../components/CaseDetails';

export default function LawyerDashboard({ account }) {
    ...
    const loadMyCases = async () => {
        try {
            ...
            for (let i = 1; i <= Number(counter); i++) {
                ...
                // Check if lawyer is advocate1 or advocate2
                const adv1 = result.advocate1.toLowerCase();
                const adv2 = result.advocate2.toLowerCase();
        ...
        <div className="min-h-screen p-6 md:p-12 text-slate-100">
            <div className="max-w-7xl mx-auto space-y-8">
                ...
                <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-700/50 pb-6">
                    <div>
                        <h1 className="text-3xl font-bold flex items-center gap-3"><Briefcase className="text-purple-400" /> Lawyer Dashboard</h1>
                        <p className="text-slate-400 mt-1 text-sm">Account: <span className="font-mono bg-slate-800/80 px-2 py-0.5 rounded text-purple-300">{account}</span></p>
                    </div>
                ...
                <div className="glass-card p-6 rounded-2xl overflow-hidden mt-8">
                    <h2 className="text-xl font-semibold mb-6 flex items-center gap-2 border-b border-slate-700/50 pb-4"><Search size={20} className="text-purple-400" /> My Assigned Cases</h2>
                    ...
                    {loading ? (
                        ...
                        <div className="py-16 text-center text-slate-500 bg-slate-800/30 rounded-xl border border-dashed border-slate-700">
                            ...
                            <p className="text-sm">You have not been assigned to any properties yet. Please contact the administrator.</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {cases.map((c) => (
                                <div
                                    ...
                                    className="bg-slate-800/50 border border-slate-700/50 hover:border-purple-500/50 hover:bg-slate-800 transition-all cursor-pointer p-5 rounded-2xl group flex flex-col justify-between"
                                >
                                    <div>
                                        <div className="flex justify-between items-start mb-3">
                                            <span className="text-lg font-bold font-mono text-purple-300 group-hover:text-purple-400 transition-colors">#{c.caseId}</span>
